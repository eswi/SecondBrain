...
//
// ### ⛔ 왜 위험한가 — **이 파일이 자기를 잘못 적고 있다** (2026-08-21 훑기에서 나왔다)
//    바로 아래 줄이 *"원칙: inbox.md는 읽기만 한다"*라고 말하는데 **사실이 아니다.**
//    `writeToFile()`이 File System Access API로 **inbox.md를 실제로 덮어쓴다**
//    (`handle.createWritable()`), `confirmAllPending()`이 그것을 여러 항목에 돌린다.
//    쓰는 것은 **due·resurface**다(type은 안 쓴다).
//    ⛔ **그것이 2026-08-18 축 결정을 되돌린다** — 「미확정에는 시점이 붙지 않는다」.
//       inbox.md 항목에는 `confirmed`가 없으므로 여기서 확정하면
//       **「미확정인데 날짜가 있는」 항목**이 생긴다(앱이 금지한 상태다).
//    ⚠️ 그리고 그 상태는 `docs/native/sample-pending-checks.md` A그룹의 「여는 조건」과 겉모습이 같다 —
//       ⛔ **이 경로로 만들어진 것은 표본으로 쓸 수 없다**(그 문서에 적어 뒀다).
//    ★ **훑지 않았으면 안 나왔다.** 머리주석이 스스로 「읽기만 한다」고 말하고 있었다.
//
