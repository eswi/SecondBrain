...
- **③ 저장 위치 = 사진 EXIF에만, 볼 때 읽음 (그릇엔 안 박음):**
  - **텍스트 그릇(조각 파일)은 기기 간 동기화**되지만 **사진은 기기에만.** GPS 좌표를 그릇에 박으면 **사진은 안 올라가는데 좌표만 iCloud로 동기화** = 민감정보 누출.
    > ⚠️ **2026-08-20 — 이 근거의 한쪽이 없어졌다. 결정은 안 바뀐다.**
    > 「사진은 안 올라가는데 좌표만 올라간다」가 근거였는데 **이제 사진도 올라간다**(`media-icloud-design.md`).
    > **그래도 GPS 좌표는 여전히 사진 EXIF 안에만 둔다** — 그릇에 박지 않는다.
    > (근거 하나가 사라진 것이지 **반대 근거가 생긴 것이 아니다.** 좌표를 그릇에 박으면
    > 사진을 안 받은 기기·검색·백업 텍스트에까지 기계값이 퍼진다 — 그건 그대로 피한다.)
...
> ### ⚠️ **뒤집혔다 (2026-08-20) — 본체 정정은 단계 6에서**
> 자료(음성·사진) **파일은 이제 iCloud에 있다.** 폰의 **135개가 2026-08-20 00:14에 이관됐다**
> (`iCloud Drive/SecondBrain/{audio,photo}/`). **아래 이 절의 서술은 그 전의 것이다.**
>
> **★ 한 말에 두 뜻이 붙어 있었고 한쪽만 뒤집혔다 — 가르지 않으면 단계 6에서 원칙까지 함께 지운다:**
>
> | | |
> |---|---|
> | ⛔ **뒤집혔다** | 「자료 **파일**이 기기에만 있다」 — 지금 iCloud에 올라간다 |
> | ✅ **유지된다** | 「**STT가 음성을 서버로 보내지 않는다**」(온디바이스 강제) — **그대로다.** 이건 §7 「외부 전송 최소화」의 뜻이고 저장 위치와 다른 사실이다 |
> | ✅ **유지된다** | 확정(finalize) 목적지는 **여전히 로컬** · **로컬 사본을 지우지 않는다** · 포인터 필드는 **불변 성역** |
>
> **왜 iCloud로 갔나:** 폰을 잃으면 자료를 **전부** 잃었다(텍스트만 살았다).
> **근거·전말: `docs/native/media-icloud-design.md`** (뒤집을 자리 목록 = 그 문서 §12).
