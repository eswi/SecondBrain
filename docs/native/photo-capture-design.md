...
- **② 여러 장:** 지금은 **1장.** 파일명 `<uuid>.jpg`. (여러 장은 `<uuid>-N.jpg` + 쉼표 목록으로 나중 확장 — create 블록 값이 줄 끝까지라 목록도 파서 안전.)
  ...
  > ### ⛔⛔ **이 예고를 근거로 삼지 말 것 — 병합을 안 따졌다** (2026-08-23 경고 · 예고는 지우지 않는다)
  > **위 괄호는 「파서가 견디나」만 따졌다.** ⛔ **병합은 안 따졌다.**
  > **병합은 항목별·필드별 LWW다**(`merge-design.md` §3 — *"같은 필드 충돌만 HLC 최신 승"*).
  > **그러므로 「쉼표 목록」을 한 필드에 담으면:**
  > **두 기기가 각각 사진을 한 장씩 붙였을 때 한쪽 목록이 통째로 지고 — 사진이 사라진다.**
  > ⚠️ **「다른 필드는 둘 다 산다」는 규칙이 여기서는 안 걸린다** — 같은 `photo` 필드이기 때문이다.
  >
  > ✅ **손실 없는 후보:** **자료마다 별도 필드**(`photo.<자료id>`) — 서로 다른 필드라 **둘 다 산다**.
  > ✅ **2026-08-23에 ㉯로 정해졌다**(사용자) — **자료마다 별도 필드.** 세 안과 대가는
  > `media-expansion-design.md` **§3-V-1**, 결정·실측·남은 것은 **§3-W**.
  > ⛔ **그러므로 위 괄호의 「`<uuid>-N.jpg` + 쉼표 목록」은 쓰지 않는다.**
  > ⚠️ **자료 id 꼴에 제약이 있다** — **소문자 16진(하이픈 없음)**. `UUID().uuidString`은 **키에 못 쓴다**(§3-W-2).
  >
  > ★ **왜 이 경고를 여기 붙이나:** **예고가 문서에 있으면 「이미 정해진 것」으로 읽힌다.**
  > 다음 세션이 이 줄을 근거로 삼으면 **자료를 잃는 길로 간다.** **지우면 왜 안 되는지가 안 남는다.**
