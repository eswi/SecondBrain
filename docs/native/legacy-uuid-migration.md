# 레거시 → UUID 일괄 마이그레이션 — 설계 + 실행 기록

> ✅ **실행 완료(2026-07-20): ①백업 → ②dry-run → ③apply → ④원본 교체 → ⑤실기기 확인까지 완료.**
> **⑥ 레거시 처리 코드 제거는 며칠 실사용 뒤 별도 단계로 미룸(아직 안 함).**
> 실데이터(iCloud `inbox*.md`)를 변형하는 작업이라 `merge-design.md`급으로 신중히 다뤘다.
> 관련 정본: `merge-design.md`(병합 규칙 — **이 마이그레이션으로 안 바뀜**), `edit-policy.md`(§7 레거시·§8 원문 잠금).
> 작성·실행 2026-07-20 (MacBook, 단일 기기).
...
| 항목 | 값 |
|---|---|
| `inbox.md` 레거시 줄(id 없는 create) | **71개** (voice 62 · web 9) — 전부 고유(중복·충돌 0). 사양서 "68"은 오래된 수 → `edit-policy.md §7` 71로 갱신함 |
...
## 8. 실행 순서 (각 단계 후 정지·보고) — 실행 기록
```
✅ ①백업(로컬+zip)  — ~/SecondBrain-backup-20260720-1912/ (+ .zip), 원본=백업 해시 일치
✅ ②dry-run          — 71 전부 고유(중복·충돌 0), 고아 참조 0, before==after 6/6 통과
✅ ③apply            — ~/SecondBrain-migrated-20260720-1936/, 산출물 재검증 6/6, legacy 토큰 0
✅ ④원본 교체        — 교체 전 드리프트 검증(원본=백업 해시, 새 조각·placeholder 없음) 후 md 2개 교체, 원본=산출물 일치
✅ ⑤실기기 확인      — 71개·기억하기 11·원칙 7 그대로. --stats로 현재=백업 전 칸 동일 재확인.
⬜ ⑥레거시 코드 제거 — 며칠 실사용 뒤 별도 단계(아래 §10). 아직 안 함.
```
검증 수치: 역대 전체 91 = 살아있는 50 + 보관 41(삭제 24 + discard 15 + 완료 2). 마이그레이션 전후 동일.
...
## 11. 결정 로그 (D)
- D1(닫힌 집합 68 vs 71): **71 확정**(dry-run: 71 전부 고유, 중복·충돌 0). `edit-policy.md §7` 68→71 갱신 완료.
- D2(iPhone 기록 여부): **없음 확인**(2026-07-20, 전 이벤트 단일 dev-155f307a). 교체 직전 드리프트 검증도 통과.
- D3(device 동결): **동의**.
