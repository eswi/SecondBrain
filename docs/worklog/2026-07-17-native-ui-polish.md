...
- **필터 범위 재결정(사용자)**: "최근 들어온 것" → **"기억 목록"**으로 개명. **필터는 '기억 목록'에만 적용** — 원칙·곧 닥칠 것은 각자 "항상 걸린 분류"(principle 타입 / 시점 있음)라 필터 무관하게 항상 전체 표시. 원칙·시점 항목은 기억 목록에 안 보이고, 재분류(원칙 해제/시점 제거)되면 기억 목록으로 내려옴. 필터 칩에서 '원칙' 제거(자체 섹션 상시 표시라 필터 대상 아님). **실기기 검증 완료(2026-07-18)** — 개명·필터 범위 정상 동작.
