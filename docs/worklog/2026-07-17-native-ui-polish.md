...
- 실기기: iCloud 68개 로드·**레거시 분류 변경**·삭제(=버림 포함 10) 확인. 접힘/요약은 실기기 피드백으로 폐기.
- **필터 검증(시뮬레이터, 사후)**: 초기 worklog엔 필터 확인이 빠져 있었음. 사후 점검에서 필터 자체는 정상 확인. ⏳ 실기기 필터 탭은 아직 미확인.
- **필터 범위 재결정(사용자)**: "최근 들어온 것" → **"기억 목록"**으로 개명. **필터는 '기억 목록'에만 적용** — 원칙·곧 닥칠 것은 각자 "항상 걸린 분류"(principle 타입 / 시점 있음)라 필터 무관하게 항상 전체 표시. 원칙·시점 항목은 기억 목록에 안 보이고, 재분류(원칙 해제/시점 제거)되면 기억 목록으로 내려옴. 필터 칩에서 '원칙' 제거(자체 섹션 상시 표시라 필터 대상 아님). 시뮬레이터에서 정보 필터→원칙·곧닥칠 전체 유지+기억목록만 정보로 확인. ⏳ 실기기 재검증 필요.
