...
- **필터 검증(시뮬레이터, 사후)**: 초기 worklog엔 필터 확인이 빠져 있었음. 사후 점검에서 (a)필터 자체는 정상(약속→곧닥칠 약속만·최근 0), (b)**버그 발견**: 원칙 섹션이 필터와 무관하게 항상 표시되고 '원칙' 필터 시 최근에 중복 → **수정**: `showsPrincipleSection`(전체/원칙 필터일 때만 원칙 섹션), '원칙' 필터의 `filteredInbox=[]`(중복 방지). 약속·원칙 필터 스크린샷으로 확인. ⏳ 실기기 필터 탭은 아직 미확인.
