...
> **재개 한 줄:** 상세 화면(§6)·"기억하기" 재정의·커스텀 재확인 대화상자·macOS 빌드 수정 + **"확정→기억하기" 용어 정합화(문서 3종 + 앱 라벨)** + **대시보드·설정 라벨을 세 영역 이름("새 기억·살아있는 기억")으로 정합**까지 main에 커밋·push 완료(`779cd98`), 실기기 확인됨. **다음: ①"총 기억" 대시보드 정의 재결정 ②수집([+]) 실구현.**
...
## 5. 상태
- main = origin/main = `779cd98`. working tree clean. worktree·브랜치·열린 PR 잔재 없음.
