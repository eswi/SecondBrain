...
> **재개 한 줄:** 상세 화면(§6)·"기억하기" 재정의·용어 정합화·대시보드 라벨·원칙 기능·앱 안 STT 음성 수집 + **액션 버튼/단축어 연동(App Intent) + iPhone 전용 설정**까지 main 커밋·push 완료(`16b90a6` + 문서 정합), 실기기 확인됨(액션 버튼→앱→STT). **다음: ①자동 분류(Claude API — 수집 항목 자동 분류) ②"총 기억" 대시보드 정의 재결정 ③원칙 각인 방식(§3 큰 과제)·원칙 0개 밴드 숨김 검증.**
...
## 4-f. 액션 버튼/단축어 연동 (`16b90a6`)
- **App Intent 방식**(메인 타깃): `CaptureMemoryIntent`(openAppWhenRun) + `SecondBrainShortcuts`(AppShortcutsProvider) → 단축어·액션 버튼·Siri에 자동 노출. **별도 App Extension·특수 entitlement 불필요 = 무료 서명 유지.**
- `CaptureLauncher`(공유 신호) → RootView가 관찰해 새로운 기억 탭 + 기존 `CaptureSheet`(STT 자동 시작) 재사용.
- `TARGETED_DEVICE_FAMILY=1`(iPhone 전용) — 사양서 §0-A와 정합, iPad 방향 경고 제거. iPad 확장 시 "1,2"로 되돌림(아키텍처는 안 막음, 되돌림 시 CaptureDevice 실제 모델 판별로 교체).
- 실기기 확인: 액션 버튼 → 앱 열림 → STT 시작. (콘솔의 키보드/RTIInputSystemClient 로그는 무해한 시스템 노이즈.)

## 5. 상태
- main = origin/main = `16b90a6`(+문서 정합 커밋). working tree clean. worktree·브랜치·열린 PR 잔재 없음.

## 6. 다음 할 일
1. **자동 분류(Claude API)** — 수집된 새 항목 자동 분류(계속 미뤄둔 다음 단계).
2. **"총 기억" 대시보드 정의 재결정** (현재 = 살아있는 전체).
3. 원칙 **각인 방식**(§3 큰 과제) · 원칙 0개 밴드 숨김 검증.
4. 보류: 완전 삭제(edit-policy §8), 완료취소 재확인(§5) 강제, 원문 성역 코드 강제, 완전 이력(§4-4), 원문 여러 줄 보존(현재 수집은 줄바꿈→공백), 대화상자 헬퍼 앱 전역 공용화, iPad 확장(TARGETED_DEVICE_FAMILY·레이아웃·기기 모델 판별).
