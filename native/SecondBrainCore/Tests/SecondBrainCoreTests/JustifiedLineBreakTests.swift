...
final class JustifiedLineBreakTests: XCTestCase {
    ...
    /// 경계를 **시험으로 못 박는다** — 빈칸으로 떨어진 부호는 줄 앞에 올 수 있다.
    ///
    /// **둘을 갈라 적는다** (2026-08-21):
    /// - **사실:** 이것은 **규칙 밖이고 지금 동작이 맞다.** 원칙 ①은 「부호 *바로 앞* 글자」인데
    ///   빈칸이 끼면 부호의 바로 앞은 **글자가 아니다.**
    /// - **결정:** ⛔ **넓히지 않기로 했다** (사용자). 근거 둘 —
    ///   ① **규칙이 복잡해진다**(지금은 한 줄인데 빈칸까지 다루면 「빈칸 앞의 글자를 찾아서」가 된다)
    ///   ② **실데이터에 그런 문장이 없다.** → **나중에 눈에 걸리면 그때 연다.**
    ///
    /// ⚠️ 그래서 이 시험이 **깨진다면** 구현이 틀린 것이 아니라 **누군가 범위를 넓힌 것**이다 —
    /// 그때는 위 결정을 다시 본다.
