...
public enum MediaMigrationText {
    ...
    /// **어디에도 없을 때** (`MediaAvailability.absent`) — 종류마다 다르다.
    ///
    /// **2026-08-20에 바꾼 말이다.** 옛 문구는 「원본 음성 있음 · **이 기기엔 없음**」이었다.
    /// 그때는 맞았다 — 자료가 기기에만 있었으므로 **다른 기기엔 있을 수 있었다.**
    /// 자료가 iCloud로 간 뒤 `absent`의 뜻이 **「이 기기에도 iCloud에도 없다」**로 좁아졌는데,
    /// 옛 말은 사용자가 **「다른 기기에서 열면 되겠네」로 읽는다** — 갈 곳이 없는데 있다고 말한다.
    ///
    /// **「아직 안 올라감」을 안 고른 이유**(사용자, 2026-08-20): **원인을 단정한다.**
    /// `downloadFailed`가 원인을 안 짚는 것과 같은 경계다 — 왜 없는지는 이 자리에서 알 수 없다.
    ///
    /// ⚠️ **`downloading`과 같은 꼴로 여기 둔다** — 음성·사진 두 군데 문자열로 두면
    /// **한쪽만 고쳐져 말이 갈린다**(`photoPinName`을 여기 둔 것과 같은 이유).
    public static func missing(_ kind: MediaKind) -> String {
        switch kind {
        case .audio: return "원본 음성 있음 · 파일을 못 찾음"
        case .photo: return "원본 사진 있음 · 파일을 못 찾음"
        }
    }
