...
struct DetailView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showAddSheet, onDismiss: {
            ...
            case .camera:
                ...
                // ⛔ **한 장만 찍는다** — 연달아 찍기는 되돌렸다(`SystemCamera` 머리주석).
                //   **여럿은 앨범이 맡는다**(여기서는 장마다 바로 op으로 붙는다).
    ...
    // ✅ **문구 둘은 정해졌다**(2026-09-21 사용자 결정 「지금 값 그대로」 · 설계 §4): 자리표시자 「해시태그 추가」 · 나열 제목 「이 분류에서 쓴 해시태그」.
    ...
    private var expiredBanner: some View {
        ...
        if ItemSchedule.isExpired(fresh, now: Date()) {
            ...
            .padding(12)
            .background(Palette.expired.opacity(0.12), in: RoundedRectangle(cornerRadius: 10))
        }
    }
    ...
    ///
    /// 사용자: *"주차위치를 기억하는 때는 처음에 항상 「다시 보기」 날짜가 바로 그 날, 즉 오늘이야."*
    ///
    /// ## ⛔⛔ 저장까지 하지 않는다 — **손으로 하던 그 순서를 그대로 자동으로만 한다**
    /// 사용자: *"원래는 그 다음에 내가 날짜를 오늘로 설정하고, 설정되면 날짜가 **노란색**으로 표시되고,
    /// 아래에 같은 노란색으로 「저장하지 않은 수정이 있어요」가 작게 나오고, **[저장] 버튼이 활성화**됐어.
    /// 지금은 자동으로 날짜가 들어갔는데 **이 과정이 안 나와.** 그러니 내가 저장을 누르지도 못 해.
    /// **기존 순서를 자동으로 해줘. 내가 손으로 [저장]을 눌러서 나갈게."*
    ///
    /// ⛔ **옛 꼴(지우지 않고 적어 둔다 · 두 번 틀렸다):**
    /// **①** 커밋에 실어 보냈다 → `commitEdits`가 **미기억이라 버렸다**(그 자리 주석에 전말).
    /// **②** 그래서 **기억하기를 먼저** 하도록 순서를 바꿨다 → **값은 저장됐지만 뜻이 아니었다.**
    /// **사용자는 「자동 저장」이 아니라 「자동 입력」을 원했다** — 저장은 사람이 누른다.
    /// ★ **둘 다 「어떻게 저장하나」를 풀고 있었고, 물음은 「저장할 것인가」였다.**
    ///
    /// ## 그래서 하는 일은 한 줄이다
    /// **draft에만 넣고 기준선은 안 옮긴다.** 그러면 이 파일이 이미 쓰는 관용구대로
    /// **노란 날짜 + 「저장하지 않은 수정이 있어요」 + [저장] 활성화**가 저절로 따라온다
    /// (이 파일 머리의 *"draft만 옮기고 기준선을 안 옮기면…"* 참조).
    ///
    /// **넣는 조건 셋** — 하나라도 어긋나면 안 건드린다:
    /// ① 지금 고른 분류가 **주차 위치** ② 화면의 「다시 보기」가 **비어 있다**
    /// ③ **저장된 값도 비어 있다**(사람이 넣었다 지운 것을 되살리지 않는다).
    ///
    /// ✅ **분류가 그 칸을 쓴다는 보증은 Core에 있다** —
    ...
    private func remember() {
        ...
        isRemembered = true
        // ★ **여기서부터가 자동 입력이다** — 위에서 기준선을 비운 **뒤**라야
        //   이 한 줄이 「저장 안 된 수정」으로 잡힌다. ⛔ 순서를 앞으로 옮기지 말 것.
        if normalizedType == ClassSpecCatalog.parkingKey,
           (resurface ?? "").isEmpty, (baseline.resurface ?? "").isEmpty {
            resurface = ItemSchedule.dayString(Date())
        }
    }
