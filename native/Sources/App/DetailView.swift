    ...
    private func timeDialog(_ field: TimeField) -> some View {
        ...
        StandardDialog(title: title) {
            VStack(spacing: 12) {
                ...
                // ★ **달력은 제 크기 그대로 둔다 — 높이를 주지 않는다**(2026-08-10, 세 번째 시도).
                //
                // **앞선 두 번이 왜 틀렸나:** `.frame(height: 330)`으로 못박았더니 달력이 **틀 안에서**
                // 어떤 때는 꽉 차게(≈304pt) 어떤 때는 오그라들어 가운데로(≈236pt) 그려졌다. `maxHeight:
                // .infinity`를 얹어도 마찬가지였다. **틀을 주는 한 「남는 높이를 쓸지 말지」라는 자유가 남는다.**
                //
                // **`fixedSize(vertical:)`는 그 자유를 없앤다** — 제안된 높이를 **무시하고** 언제나
                // 제 이상 크기로만 그린다. 늘어날 일도, 오그라들 일도, 가운데로 몰릴 일도 없다.
                //
                // **처음에 대화상자가 들썩였던 진짜 이유도 이것으로 설명된다:** 시각 줄이
                // **켜짐=피커 / 꺼짐=글자**로 높이가 달라졌고, 그만큼 **남은 높이가 달라져 달력이 늘었다 줄었다** 했다.
                // 시각 줄은 이미 34pt로 못박았으므로(아래), 달력에서 틀을 걷어도 대화상자는 안 흔들린다.
                // 남는 변수는 **달의 주 수(5·6주)** 뿐인데, 그건 달을 넘길 때만이고 **같은 달 안에서는 안 변한다.**
