    ...
    private func timeDialog(_ field: TimeField) -> some View {
        ...
        StandardDialog(title: title) {
            VStack(spacing: 12) {
                ...
                // ★ **늘어난 큰 달력을 택하고, 크기 변함은 감수한다**(2026-08-10 사용자 결정 「다」).
                //
                // **네 번 시도했고 못 닫았다** — 과정은 확인표 G-12에 표로 남겼다:
                // ① `frame(height:330)` ② `+ maxHeight:.infinity` ③ `fixedSize(vertical:)`(틀 걷음)
                // ④ + 대화상자 폭 300→360. **③도 ④도 변함을 없애지 못했다.**
                //
                // **실측이 바로잡은 것:** 같은 2026년 8월·같은 6줄인데 **행 간격이 34.0 ↔ 46.3pt**로 갈렸고,
                // 대화상자 구분선은 **248·604·664pt로 동일**했다. → **주 수 문제가 아니다**(그렇게 설명했던 것은
                // 틀렸다). 같은 달에서 **날짜만 눌러도** 변한다 — 처음엔 늘어나고, 누른 뒤엔 제 크기로 돌아가
                // 가운데 정렬된다(위아래 50pt 빈 띠).
                //
                // **그래서 틀을 다시 준다.** 틀이 있어야 **늘어난 큰 달력**(행 46.3pt)이 나온다 —
                // 큰 달력은 곧 «늘어난 상태»이고, 늘리기를 없애면 작은 달력으로만 고정된다. 둘을 함께 가질 수 없다.
                // **값은 안 잃고 동작도 정상이라 급한 자리가 아니다.** 다음에 볼 때 후보(달력 직접 그리기 ·
                // `.wheel` · 높이 못박고 잘라내기)도 G-12에 적어 뒀다.
