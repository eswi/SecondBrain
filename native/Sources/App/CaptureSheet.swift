...
/// | **강제 종료**(앱 전환기에서 밀어 올림 · 2026-09-17) | `onDisappear`(✅ **확인됨** — 폰 `draft.log` 01:42:18 `discard … closed=false`가 마지막 persist 4초 뒤·재시작 restore 6초 전에 찍혔다) | `discardTemps()`가 **초안 폴더로 옮겨진 사진까지** 지웠다 → 「쓰다 만 기억」에 글은 남고 **사진만 사라졌다** |
///
...
struct CaptureSheet: View {
    ...
    private var sheetBody: some View {
        ...
        .onDisappear {
            ...
            // ⛔⛔ **덮는 화면이 뜰 때도 여기 온다**(머리주석 표) — 그때 정리하면 **녹음·사진이 사라진다.**
            //    ★ **2026-09-03에 하나가 늘었다: 뷰어**(`showViewer` · `fullScreenCover`).
            //    **카메라와 같은 성질**이다 — 화면을 통째로 덮으므로 이 시트가 「사라졌다」고 불린다.
            //    ⚠️ **어제 뷰어를 붙일 때 이 가드를 안 세웠다** — 머리주석이 **정확히 이 함정을
            //    경고하고 있었는데도** 그랬다(경고를 읽고도 새 덮개에 적용하지 못한 것이다).
            //    ⛔ **다음에 덮는 화면을 하나 더 붙이면 이 줄을 함께 고친다.**
            //    ⚠️ **`.sheet`(앨범·URL·앱 안 보기)도 같은지는 못 잼** — 덮개가 아니라 카드라
            //    이 콜백이 안 오는 것으로 보이지만 **재지 않았다.**
            guard !cameraOpen, !showViewer else { return }
            location.stop()
    ...
    private func save() {
        ...
        // ★★ **핫키로 들어왔으면 신호를 직접 내린다** (2026-09-03에 고쳤다).
        //
        // ⛔⛔ **여기가 결함이었다 — [저장 후 편집하기]를 눌러도 아무 일도 안 났다**
        //    (사용자: *"이 상태에서 '저장 후 편집하기' 버튼을 눌러도 아무일도 일어나지 않음.
        //    이 현상 처음이 아니야."*).
        //    **뿌리로 그려졌을 때 `dismiss()`는 아무 일도 안 한다**(띄운 것이 아니다) —
        //    그래서 **기억은 저장되는데 화면이 그대로였다.** 조용해서 「안 됐다」로 보였다.
        // ★ **`leaveNow`는 이 줄을 갖고 있었고 여기만 없었다** — **한쪽만 고친 자리다.**
        //    ⚠️ **`cancelledOut`은 켜지 않는다** — 그것은 「앱 밖으로」라는 뜻이고,
        //    저장은 **앱 안에 남아 상세로 이어지는** 길이다(`InboxView`가 `showCapture`가
        //    내려가는 것을 보고 상세를 민다).
        #if os(iOS)
    ...
    /// **부른 자리는 `onDisappear`** — 강제 종료 때 화면이 해체되며 온다(✅ **2026-09-17 01:42 폰 `draft.log`로 확인** — `discard … kept=1 closed=false` 뒤 `restore … found=1` · 다섯 번 다 같았다).
    ...
    @ViewBuilder private func mediaAddPlumbing<V: View>(_ content: V) -> some View {
        content
            ...
            .sheet(isPresented: $showAlbum) {
                // ★ **여러 장이 온다**(2026-09-03) — **고른 순서 그대로 쌓는다.**
                AlbumPicker { temps in draftPhotos.append(contentsOf: temps) }
            }
