...
///
/// ## ⛔ **[저장]은 목록으로 돌아온다 — 상세로 자동 이동하지 않는다** (2026-08-31 사용자 판정)
/// **옛 결정(2026-08-30 · 뒤집혔다 · 지우지 않는다):** *"저장하기 누르면 지금 저장된 기억의
/// '상세 화면'으로 넘어가게 해줘. 거기서 내용이나 자료 추가하고 '기억하기'까지 선택하게 이어지는 것이
/// 좋겠어."* → `model.openDetailId`로 `InboxView`가 밀었고 **`06aff97`에서 판정을 통과했다.**
/// ⛔ **그런데 자료 카드(`22d62ad`)가 들어오자 깨졌다** — [저장]이 자료마다 `load()`를 돌려
/// **닫힘과 겹치는 구간에서 미는 것이 반쪽만 적용되고 내비 바에 back chevron 자국이 남았다**
/// (사용자: *"< 아이콘이 좌측 상단에 2개가 나와"*).
/// ✅ **사용자 판정:** *"수집 화면에서 바로 상세화면으로 가질 않더라. 그건 괜찮아. 유지하자."*
/// **상세로 가는 길은 기억을 누르는 것 하나다.** 전말 → `InboxView`의 ⛔⛔ 블록.
///
...
struct CaptureSheet: View {
    ...
    private func save() {
        ...
        #endif
        saved = true
        // ⛔ **상세로 자동 이동하지 않는다** — 걷어냈다(2026-08-31 사용자 판정 · 머리주석).
        //    `newId`는 위에서 **자료를 붙이는 데만** 쓴다.
        _ = newId
        dismiss()
    }
