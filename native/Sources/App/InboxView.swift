...
struct InboxView: View {
    ...
    private var promptBody: String {
        ...
        // ★ **문구를 사용자가 골랐다**(2026-08-31 · 후보 ⓐ) — 옛 문구는 **두 군데가 틀려 있었다:**
        // 아이콘이 **마이크가 아니라 `+`**이고, 자리가 **아래가 아니라 위쪽 우측**이다.
        // ⛔ **옛 서술:** *"아래 마이크를 눌러 말하거나 적어보세요."*
        // ⚠️ **이 문장은 `folderLink == .empty`에서만 보인다** — 폴더는 열렸는데 `inbox*.md`가 **0개**일 때
        // (새 폴더를 막 골랐을 때). 그래서 **실사용에서는 거의 안 보이고**, 사용자도 못 찾았다.
        default:           return "위의 + 를 눌러 말하거나 적어보세요."
        }
    }
}
