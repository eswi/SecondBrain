...
final class InboxModel: ObservableObject {
    ...
    /// - 미리 알림을 **시점으로** 안 쓰는 분류(지식·아이디어·원칙·추억 · 그리고 **정보** — 유효 기간은 시점이 아니다)에선
    ///   미루기가 무의미 → UI에서 액션을 숨겼지만(1차 방어), 여기서도 조용히 막는다(휴면 값이 써지지 않게).
    ///   근거: §7(c) — 게시·알림·미루기는 `schedules`를 본다(2026-09-14).
    func defer7(_ item: ResolvedItem) {
        guard ClassSpecCatalog.schedules(item.type, .resurface) else { return }   // 안전망(액션 숨김이 1차)
    ...
    ///
    /// ## ⛔⛔ 덫 — **「기억하기와 함께 정해지는 값」을 여기로 보내면 사라진다** (2026-09-13에 물렸다)
    /// 이 걸러내기는 **부르는 시점의 `confirmed`**를 본다. `[기억하기]`는 **커밋을 먼저** 하므로
    /// 그 순간 항목은 **아직 미기억**이고, `raw`·`type`이 아닌 값은 **조용히 떨어진다.**
    /// 주차의 「다시 보기 = 오늘」을 여기로 실었다가 정확히 그렇게 잃었다.
    /// ✅ **답은 순서를 바꾸는 것이 아니었다** — 그런 값은 **draft에만 넣고 기준선을 안 옮긴다.**
    /// 그러면 「저장하지 않은 수정이 있어요」가 뜨고 **사람이 [저장]을 누른다**
    /// (2026-09-13 사용자 결정 · `DetailView.remember()`).
    ...
    /// - Returns: 만들어진 항목의 **id**(저장 안 했으면 nil) — 부르는 쪽이 **그 항목에 자료를 붙이려고**
    ///   쓴다(`CaptureSheet.save()` → `addPhoto`·`addURL`). 파일 이름에 항목 id가 들어가므로
    ///   **자료를 붙이려면 항목이 먼저 있어야 한다.**
    ///   ⛔ **옛 서술(2026-08-31 정정):** *"부르는 쪽이 그 상세 화면으로 이어가려고 쓴다
    ///   (`CaptureSheet.save()` → `openDetailId`)"* — **그 자동 이동은 걷어냈다**(`CaptureSheet` 머리주석).
    ///   ⚠️ **id를 돌려주는 것 자체는 그대로 필요하다** — 쓰임이 「이동」에서 「자료 붙이기」로 옮겨졌다.
    ///
