...
enum URLPreview {
    ...
    /// ★ **뽑는 규칙의 판.** 규칙이 바뀌면 이 값을 올린다 — 그러면 옛 캐시가 **안 읽힌다.**
    ///
    /// ⛔⛔ **판을 올려도 「다시 뽑히지」는 않는다 — 2026-08-26에 갈렸다.**
    /// `fetchOnce`를 부르는 자리는 **`addURL` 하나뿐**이다(결정 D — 붙일 때 한 번). 그래서 판을 올리면
    /// 이미 붙어 있는 URL은 **캐시를 잃고 영구히 ①꼴**이 된다. **나아지는 게 아니라 사라진다.**
    /// ★ **데이터가 이미 증언하고 있었다:** 캐시에 **판 없는 파일 둘**(08-24 23:4x·23:5x)이
    /// 판이 생긴 뒤에도 그대로 있었다 — `sweepOldRules`도 `fetchOnce` 안에만 있어 **부를 계기가 없다.**
    /// ✅ **그래서 갱신 수단을 따로 만들었다** — **`refetch`**(사용자가 네모를 길게 눌러 부른다).
    /// ⛔ **그것이 있으므로 판을 올릴 이유가 없어졌다** — 판을 올리면 **전부** 잃고 하나하나 눌러야 한다.
    /// **판은 「옛 캐시를 안 읽는다」는 뜻으로만 남긴다.**
    ///
    /// *(옛 서술 — 지우지 않고 남긴다: "규칙이 바뀌면 이 값을 올린다 — 그러면 옛 캐시가 **안 읽히고
    /// 다시 뽑힌다**. ⛔ 없으면 규칙을 고쳐도 이미 붙은 URL이 새 규칙을 못 받는다 … 판이 그 매듭이다."
    /// **뒤엣말이 틀렸다** — 매듭은 판이 아니라 **다시 뽑을 계기**였다.)*
    ///
    ...
    /// ★ **사용자가 눌러서 다시 뽑는다** — 「미리보기 다시 받기」(2026-08-26 사용자 결정 · 문구도 사용자가 골랐다).
    ///
    /// ## 왜 필요했나 — **갱신 수단이 없었다**
    /// 규칙을 고쳐도(08-25에 두 번) **이미 붙어 있는 URL은 옛 캐시를 그대로 쓴다** — 뽑는 자리가
    /// `addURL` 하나뿐이라 계기가 없다(위 `rule` 주석). **실측:** `wowanalytica.com`의 캐시가
    /// **171×171 · 08-25 00:18**의 것이었다(옛 판의 「늘린 아이콘」) — 페이지 캡쳐로 안 바뀌었다.
    ///
    /// ## ⚠️ 결정 D를 **가장 얕게** 건드린다 (사용자 결정 · 네 갈래 중 이것을 골랐다)
    /// 정본은 *"목록을 여는 것만으로는 아무 사이트도 연결하지 않는다"*다.
    /// **여기서 연결하는 것은 사용자가 눌렀을 때뿐이다** — **자동 연결은 여전히 0이다.**
    /// ⛔ **안 고른 길:** 상세를 열 때 자동으로(결정 D를 정면으로 넘는다) · 판을 올려 전부 버리기.
    static func refetch(_ raw: String) async {
        await fetchOnce(raw, force: true)
    }

    /// 그 사이트에 한 번 연결해 대표 그림을 받아 캐시에 둔다.
    /// **이미 해본 URL이면 아무것도 안 한다**(연결하지 않는다).
    ///
    /// - Parameter force: `true`면 **이미 해봤어도 다시 한다** — 옛 그림과 `.miss`를 **먼저 지운다.**
    ///   ⚠️ **`.miss`를 지우는 것이 핵심이다** — 안 지우면 `attempted`가 걸려 아무것도 안 한다.
    static func fetchOnce(_ raw: String, force: Bool = false) async {
        ...
        if force {
            // ⛔ **둘 다 지운다** — 그림만 지우면 `.miss`가 남아 「이미 해봤다」로 걸린다.
            if let i = imageFile(normalized) { try? FileManager.default.removeItem(at: i) }
            if let m = missFile(normalized) { try? FileManager.default.removeItem(at: m) }
        }
        guard force || !attempted(normalized) else { return }
