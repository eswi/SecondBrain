...
///
/// **2026-09-22 추가 — 해시태그 `tag.<id>` 넷(가족 ×2 · 선물 · 앱 아이디어).** 해시태그 필터 패널(`TagFilterDock`)을 시뮬레이터에서
/// 그려 보기 위한 것 — 태그가 없으면 패널에 「태그 없음」 하나만 뜬다. id는 16진 32자 꼴만 맞춘 가짜다.
///
...
/// ⛔ **뒤집혔다 (2026-08-23 · C) — 이제 필드 값이 조회에 쓰인다.** 옛 서술을 지우지 않는다:
/// *"필드 값(`sample-parking.*`)은 **표시용일 뿐 조회에 안 쓰인다** — `PhotoStore.url(forId:)`가
/// 파일명을 `"<항목 id>.jpg"`로 **직접 만든다.** 실제 파일을 심으려면 그 id로 이름을 지어야 한다."*
///
/// **지금:** 조회가 **포인터 값을 파일명으로** 쓴다(`PhotoStore.url(name:)` · 설계 §3-X).
/// 그래서 실제 파일을 심으려면 **`sample-parking.jpg`라는 이름으로** 두면 된다(항목 id가 아니다).
/// ⚠️ **파일이 없는 것은 그대로다** — 그래서 이 표본은 **`.here` 갈래를 여전히 못 밟는다**
/// (`CLAUDE.md` 계측 규칙 8의 그 자리 · 설계 §3-L).
...
enum SampleData {
    ...
    - 2026-07-13 19:00 | voice | 아이 학원 등록 마감
      type: promise
      due: 2026-07-14
      tag.0f1e2d3c4b5a69788796a5b4c3d2e1f0: 가족
    ...
    - 2026-07-17 08:00 | voice | 엄마 생신 선물 사기
      ...
      tag.1f1e2d3c4b5a69788796a5b4c3d2e1f1: 가족
      tag.2f1e2d3c4b5a69788796a5b4c3d2e1f2: 선물
    - 2026-07-16 14:30 | web | 오프라인 우선 노트 앱 아이디어
      type: idea
      confirmed: true
      tag.3f1e2d3c4b5a69788796a5b4c3d2e1f3: 앱 아이디어
